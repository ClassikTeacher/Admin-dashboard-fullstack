...
const FromEmployee = () => {
  ...
  const options: IOptions[] = reduxStore.departments.map((item) => {
    return { value: item.name, name: item.name };
  });

  async function click() {
    const departmentId = reduxStore.departments.filter(
      (item) => item.name == department
    );
    const response = await AdmindashboardService.createEmployee(
      firstName,
      lastName,
      company,
      department,
  ...
  return (
    <div>
      <form action="#" id="feedbackForm" className={styles.form}>
        ...
        <MyButton children="Submit" click={click} />
      </form>
    </div>
  );
};
